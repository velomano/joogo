...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Order Status API called with params:', { from, to, region, channel, category, sku });
    ...
    let query = sb
      .from('fact_sales')
      .select('order_id, order_status_code, order_status_name, order_status')
    ...
    if (salesData && salesData.length > 0) {
      ...
      Array.from(orderMap.values()).forEach(order => {
        ...
        const statusName = order.orderStatusName || '입금대기';
    ...
    // 카페24 API 표준 주문 상태 정의
    const statusDefinitions = {
      'N00': { name: '입금대기', color: '#6b7280' },
      'N10': { name: '결제완료', color: '#3b82f6' },
    ...
    console.log('Order status data calculated:', result);
    return NextResponse.json({ data: result });

  } catch (error) {
    console.error('Order Status API error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch order status data', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
