-- Fix board_reorder_points function to use correct original_data path
...
language sql security definer set search_path=public, analytics as $$
  ...
  inventory as (
    select 
      sku,
      avg(coalesce((original_data->>'stock_on_hand')::numeric, 0)) as stock_on_hand,
      avg(coalesce((original_data->>'unit_cost')::numeric, 0)) as unit_cost
