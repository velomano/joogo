-- 0042_fix_policies.sql
-- 정책 중복 오류 해결

set role postgres;

-- 기존 정책 삭제 후 재생성
drop policy if exists fact_sales_tenant_access on analytics.fact_sales;
drop policy if exists stage_sales_tenant_access on analytics.stage_sales;
drop policy if exists raw_uploads_tenant_access on analytics.raw_uploads;
drop policy if exists items_tenant_access on public.items;
