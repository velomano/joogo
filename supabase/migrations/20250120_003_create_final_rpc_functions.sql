...
as $$
  ...
  ranked as (
    select *,
           ...
           sum(revenue) over (order by revenue desc rows between unbounded preceding and current row) cum_rev
    from base
  )
...
as $$
  ...
  select
    l.sku,
    l.last_sale_date,
    (current_date - l.last_sale_date)::int as days_since_last
  from last_sale l
